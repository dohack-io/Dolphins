...
// Hier die Dateien eintragen in denen das Formular nicht angezeigt werden soll.
// Bitte nur den Dateinamen ohne Pfadangabe eintragen!
$FORMULAR_ANZEIGE = [
 "beispiel1.php",
 "beispiel2.php",
 "beispiel3.php",
  ...
  if (isset($_GET["id"])) {
   ...
   if ($select->rowCount() == 1) {
    ...
    print '<form name="Form" action="' . $_SERVER["SCRIPT_NAME"] . (isset($_GET["seite"]) ? '?seite=' . $_GET["seite"] : '?seite=1') . '&aktion=aendern#anker" method="post">
      ...
      <p>
       <label> Name: <input type="text" name="name" value="' . htmlspecialchars($kommentar["name"], ENT_HTML5, 'UTF-8') . '" size="25" required="required"> </label> &nbsp; 
        ' . (bewertung($kommentar["bewertung"], $STERN)) . '
      </p>

      <p>
       <label for="kommentar"> Kommentar:<br>
       <textarea name="kommentar" id="kommentar" rows="12" cols="52" required="required">' . htmlspecialchars($kommentar["kommentar"], ENT_HTML5, 'UTF-8') . '</textarea>
      </p>
...
if ($AnzahlKommentare > 0) {
 ...
 print '<fieldset class="kommentarBox" id="kommentare"> <legend> ' .
  ...
  ((count($kommentare) >= 4 && !isset($_GET["aktion"]) && !in_array(basename($_SERVER["SCRIPT_NAME"]), $FORMULAR_ANZEIGE)) ? ' - <a href="#anker">Kommentar eintragen</a>' : '') .
  '</legend>';
...
// Formular
if (!isset($_GET["aktion"])) {
 if (!in_array(basename($_SERVER["SCRIPT_NAME"]), $FORMULAR_ANZEIGE)) {
...
function smileys($kommentar) {
 $kommentar = htmlspecialchars($kommentar, ENT_HTML5, 'UTF-8');
 $smiley = array(
